...
int tcp_init(char* TCP_IP)
{
    ...
    printf("TCP: Receiver IP(s) used: %s\n", inet_ntoa(ServerAddr.sin_addr));
    printf("TCP: Receiver port used: %d\n\n", htons(ServerAddr.sin_port));
...
void tcp_close(void){
    ...
    if(closesocket(clientSocket) != 0)
         printf("TCP: Cannot close \"clientSocket\" socket. Error code: %ld\n", WSAGetLastError());
    else{
        printf("TCP: Closing \"clientSocket\" socket...\n");
    }
